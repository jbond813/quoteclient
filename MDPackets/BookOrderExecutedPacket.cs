...
namespace MDPackets
{
    public class BookOrderExecutedPacket : Packet
    {
        public BookOrderExecutedPacket(ushort len, ushort pt, byte[] pl)
        {
            rawPayload = pl;
            length = len;
        ...
        {
            return new StringBuilder().Append("book order exec ").Append(Time.ToString("HH:mm:ss.fff")).Append(" ").Append(Symbol).Append(" ").Append(ReferenceNumber).ToString();
        }
    }
}